# OpenCode Bedrock Setup (CMS)

Interactively guide a CMS engineer through configuring OpenCode to use Amazon Nova Pro on AWS Bedrock via Kion CLI (cloudtamer.cms.gov).

---

## Metadata

- **Name**: OpenCode Bedrock Setup (CMS)
- **Category**: meta
- **Description**: Interactively guides a CMS engineer through configuring OpenCode to use Amazon Nova Pro on AWS Bedrock via Kion CLI (cloudtamer.cms.gov)
- **Author**: VerdanceTeam
- **Version**: 1.0
...
- If the engineer provides a partial override (e.g. only a different account ID), keep all other CMS defaults
- This skill is CMS-specific — always remind the engineer that Anthropic models cannot be used on CMS work due to government contract restrictions
- Remind the engineer that Bedrock model access for Nova Pro must be enabled once per AWS account by an admin
...
- **`/opencode-bedrock-setup`** — guides an engineer through CMS OpenCode + Bedrock setup → see `.claude/commands/opencode-bedrock-setup.md`

Invoke with no arguments to use CMS defaults:
```
/opencode-bedrock-setup
```

Or pass overrides as arguments:
```
/opencode-bedrock-setup account_id: 999999999999, profile: my-cms, region: us-west-2
```
...
|---|---|---|
| 1.0 | 2026-03-03 | Initial version — CMS defaults, Kion CLI + Nova Pro flow |
